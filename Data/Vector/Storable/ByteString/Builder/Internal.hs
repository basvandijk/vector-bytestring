...
--
module Data.Vector.Storable.ByteString.Builder.Internal (
          ...
          where
            ...
            fillBuffer buf
              | freeSpace buf < minFree =
                  error $ unlines
                    [ "Data.Vector.Storable.ByteString.Builder.Internal.hPut: internal error."
...
untrimmedStrategy firstSize bufferSize =
    AllocationStrategy (sanitize firstSize) (sanitize bufferSize) (\_ _ -> False)
...
safeStrategy firstSize bufferSize =
    AllocationStrategy (sanitize firstSize) (sanitize bufferSize)
...
buildStepToCIOS
    ...
    -> IO (ChunkIOStream b)
buildStepToCIOS (AllocationStrategy firstSize bufferSize trim) k =
        ...
        fill !step !fpbuf = do
          ...
          where
            ...
            fullH op' minSize nextStep =
                wrapChunk op' (const $ fillNew nextStep (max minSize bufferSize))
            ...
            insertChunksH op' n lbsC nextStep =
                wrapChunk op' $ \isEmpty -> return $ YieldC n lbsC $
                    ...
                    if isEmpty
                      ...
                      else fillNew nextStep bufferSize
            ...
            wrapChunk !op' mkCIOS
              ...
              | otherwise            =
                  return $ Yield1 (VS.unsafeFromForeignPtr0 fpbuf chunkSize)
                                  (mkCIOS False)
              where
